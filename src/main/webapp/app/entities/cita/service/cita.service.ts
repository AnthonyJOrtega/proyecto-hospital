...
type RestOf<T extends ICita | NewCita> = Omit<T, 'fechaCreacion'> & {
...
export class CitaService {
  ...
  protected convertDateFromClient<T extends ICita | NewCita | PartialUpdateCita>(cita: T): RestOf<T> {
    return {
      ...cita,
      fechaCreacion: cita.fechaCreacion?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restCita: RestCita): ICita {
    return {
      ...restCita,
      fechaCreacion: restCita.fechaCreacion ? dayjs(restCita.fechaCreacion) : undefined,
    };
  }
